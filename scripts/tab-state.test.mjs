...
try {
  ...
  assert.equal(await page.getByRole('switch', { name: 'Unread badges', exact: true }).isChecked(), true)
  assert.equal(await page.getByRole('switch', { name: 'Follow new traffic', exact: true }).isChecked(), false)
  await page.locator('.settings-popover').screenshot({ path: '/tmp/sniffer-traffic-settings-off.png' })
  await page.keyboard.press('Meta+,')
  ...
  const setPreference = async (name, checked) => {
    await page.keyboard.press('Meta+,')
  ...
  const beforeFollowing = await selected()
  await sendTraffic(nextId++)
  assert.equal(await selected(), beforeFollowing, 'Following is off by default')
  assert.equal(await page.locator('.tab-unread').count(), 1, 'New traffic shows badges by default')
  await setPreference('Unread badges', false)
  assert.equal(await page.locator('.tab-unread').count(), 0, 'Disabled badges are hidden')
  assert(!await tabButton(/Socket/).getAttribute('title'), 'Disabled badges also hide unread tooltips')
  await setPreference('Unread badges', true)
  await setPreference('Follow new traffic', true)
  await page.reload()
  await rows.first().waitFor()
  devFlag = false
  await page.keyboard.press('Meta+,')
  assert.equal(await page.getByRole('switch', { name: 'Unread badges', exact: true }).isChecked(), true, 'Badge preference survives reload')
  assert.equal(await page.getByRole('switch', { name: 'Follow new traffic', exact: true }).isChecked(), true, 'Follow preference survives reload')
  await page.locator('.settings-popover').screenshot({ path: '/tmp/sniffer-traffic-settings-on.png' })
  await page.getByTitle('Toggle light/dark theme').click()
  await page.keyboard.press('Meta+,')
  await page.locator('.settings-popover').screenshot({ path: '/tmp/sniffer-traffic-settings-dark.png' })
  await page.keyboard.press('Meta+,')
  await page.getByTitle('Toggle light/dark theme').click()
  assert.equal(await pane.locator('tr[data-selected]').count(), 0, 'History snapshots are not new requests')
  for (const descending of [false, true]) {
    if (descending) await pane.locator('th.sortable').click()
    await position(descending ? 1e6 : 0)
    await sendTraffic(nextId++)
    assert.equal(await selected(), await pane.locator('tr[data-latest]').innerText(), 'New request is selected in either sort order')
    const visible = await pane.locator('tr[data-selected]').evaluate(el => {
      const row = el.getBoundingClientRect(), list = el.closest('.list-scroll').getBoundingClientRect()
      const header = el.closest('table').querySelector('th').getBoundingClientRect()
      return { top: row.top, bottom: row.bottom, listTop: Math.max(list.top, header.bottom), listBottom: list.bottom }
    })
    assert(visible.top >= visible.listTop && visible.bottom <= visible.listBottom, `Follow reveals the newest row: ${JSON.stringify(visible)}`)
  }
  await pane.locator('th.sortable').click()
  await rows.nth(10).click()
  const manualSelection = await selected()
  await sendMessages([{ type: 'event', ...traffic(nextId - 1)[1] }])
  assert.equal(await selected(), manualSelection, 'A response update does not steal selection')
  await position(0)
  await pane.getByTitle('Filter this column').click()
  await pane.locator('.filter-add input').fill('https://example.test/hidden-follow')
  await pane.getByTitle('Add filter value').click()
  if (!await pane.locator('.filter-switch input').isChecked()) await pane.locator('.filter-switch').click()
  await page.locator('.brand').click()
  await sendTraffic(nextId++, e => ({ ...e, message: { ...e.message, url: 'https://example.test/hidden-follow' } }))
  assert.equal(await selected(), manualSelection, 'Filtered arrivals do not steal selection')
  await pane.getByTitle('Filter this column').click()
  await pane.getByTitle('Disable every value').click()
  await page.locator('.brand').click()
  assert.equal(await selected(), manualSelection, 'Revealing old filtered traffic does not trigger follow')
  await switchTo(/Socket/)
  await sendTraffic(nextId++)
  assert.equal(await tabButton(/Socket/).getAttribute('data-active'), 'true', 'Follow never switches the active tab')
  assert.equal(await selected(), await pane.locator('tr[data-latest]').innerText(), 'Socket follows incoming events too')
  await pane.locator('th.sortable').click()
  await position(1e6)
  await sendTraffic(nextId++)
  assert.equal(await selected(), await pane.locator('tr[data-latest]').innerText(), 'Socket follows newest-first sorting')
  const socketVisible = await pane.locator('tr[data-selected]').evaluate(el => {
    const row = el.getBoundingClientRect(), list = el.closest('.list-scroll').getBoundingClientRect()
    const header = el.closest('table').querySelector('th').getBoundingClientRect()
    return row.top >= Math.max(list.top, header.bottom) && row.bottom <= list.bottom
  })
  assert(socketVisible, 'The followed Socket event is visible below its header')
  await rows.nth(10).click()
  const socketManualSelection = await selected()
  await sendMessages([{ type: 'event', ...entry({ type: 'socket-ack', id: `s${nextId - 1}`, payload: '[]', mocked: false }) }])
  assert.equal(await selected(), socketManualSelection, 'Socket ACK updates do not steal selection')
  await position(0)
  await pane.getByRole('button', { name: 'socketio · https://example.test', exact: true }).click()
  await sendTraffic(nextId++, e => ({ ...e, message: { ...e.message, connectionId: 'c2' } }))
  assert.equal(await selected(), socketManualSelection, 'Other connections do not trigger Socket follow')
  await pane.getByRole('button', { name: 'All', exact: true }).click()
  assert.equal(await selected(), socketManualSelection, 'Changing connections does not make old events new')
  await pane.locator('th.sortable').click()
  await switchTo(/API/)
  assert.equal(await selected(), manualSelection, 'Background arrivals do not move API selection')
  await setPreference('Follow new traffic', false)
  await sendTraffic(nextId++)
  assert.equal(await selected(), manualSelection, 'Turning follow off immediately preserves selection')
  await setPreference('Unread badges', false)
  await sendTraffic(nextId++)
  assert.equal(await page.locator('.tab-unread').count(), 0, 'Turning badges off hides existing and incoming counts')
  await page.reload()
  await rows.first().waitFor()
  devFlag = false
  await page.keyboard.press('Meta+,')
  assert.equal(await page.getByRole('switch', { name: 'Unread badges', exact: true }).isChecked(), false, 'Explicitly disabled badges stay off after reload')
  await page.keyboard.press('Meta+,')
  await setPreference('Unread badges', true)
  ...
  await socketCopyRow.click({ button: 'right' })
  await sendTraffic(nextId++)
  assert.equal(await page.getByRole('menu').count(), 1, 'Following pauses while choosing a copy action')
  await page.getByRole('menuitem', { name: 'Copy event name' }).click()
  assert.equal(await page.evaluate(() => window.copiedTraffic), 'copy:event', 'New traffic cannot change the copied event')
  await page.getByRole('menu').waitFor({ state: 'hidden' })
  await setPreference('Follow new traffic', false)
  ...
  await switchTo(/API/)
  await sendTraffic(nextId++)
  assert.equal(await selected(), await pane.locator('tr[data-latest]').innerText(), 'Capped lists still follow new requests')
  await setPreference('Follow new traffic', false)
  ...
  console.log('PASS: persisted traffic preferences, latest-request follow, unread badges, tab state, keyboard isolation, background traffic, cross-tab import results, per-item exports, push clear confirmation, and actionable empty states')
} catch (error) {
