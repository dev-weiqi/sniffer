...
- **`core` must not bundle a ktor client engine.** Every engine on the classpath registers itself for `HttpClient()` auto-discovery with the same priority, so shipping CIO inside the SDK could make the host app's engine-less `HttpClient()` pick CIO (no TLS on iOS → crash on the first https call). `Sniffer` dials the daemon with `HttpClient()` and borrows the host's engine; tests add CIO themselves. Consequence: on Android the borrowed engine (OkHttp) obeys the host's network security policy, so an https-only host must allow cleartext to the daemon host or it never connects — CIO used to bypass that policy. Documented in README / GUIDE; the samples already set `usesCleartextTraffic`.
